...
{
    public class CanvasAPISection
    {

        public CanvasAPISection(IConfiguration configuration)
        {
        ...
        public ResponseApi crear_seccion2(BodyFormSeccion curso, int course_id)
        {
            string respuesta;
            HttpWebResponse response;
            StreamReader reader;
            ...
            try
            {
                string URL_API_CANVAS = Configuration.GetSection("MySettings").GetSection("urlCANVASUPAO").Value + "/api/v1/courses/" + course_id + "/sections";
                // string URL_API_CANVAS = "https://upao.beta.instructure.com/api/v1/accounts/" + curso.account_id + "/courses";
                tRequest = WebRequest.Create(URL_API_CANVAS) as HttpWebRequest;
                tRequest.Method = "POST";
                tRequest.ContentType = "application/x-www-form-urlencoded";
                ...
                string postString = string.Format("course_section[sis_section_id]={0}&course_section[name]={1}&course_section[status]={2}", curso.sis_section_id, curso.name, curso.status);
                ...
                if ((int)response.StatusCode == 200)
                {
                    SeccionCanvas jsonCurso = JsonConvert.DeserializeObject<SeccionCanvas>(respuesta);
                    return new ResponseApi { success = (int)ResponseCode.R200, message = "OK", data = jsonCurso };
                    // return new ResponseApi { success = (int)ResponseCode.R200, message = "OK", data = jsonCurso };
                }
